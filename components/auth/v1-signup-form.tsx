...
interface V1SignupFormProps {
  isEmailVerification: boolean;
}

export default function V1SignupForm({ isEmailVerification }: V1SignupFormProps) {
  ...
  const handleOAuthSignUp = async (provider: "google" | "linkedin") => {
    try {
      await signIn(provider, {
        callbackUrl: "/complete-your-profile/jobseeker",
      });
    } catch (error) {
      toast({
        variant: "error",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/signup?type=jobseeker&e_v=true&v1=true`);
    } catch (err: any) {
      toast({
        variant: "error",
        title: "Error",
