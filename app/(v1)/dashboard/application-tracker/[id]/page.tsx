...
import ApplicationPipelineTable from "@/components/v1-dashboard/ApplicationTrackerTable";
...
export default function ApplicationTrackerIdPage() {
  ...
  const getCount = (key: string) => {
    const found = Object.keys(breakdown).find((k) =>
  ...
  const getDelta = (key: string) => {
    const found = Object.keys(deltas).find((k) =>
      k.toLowerCase().includes(key),
    );
    if (!found) return undefined;
    const val = deltas[found];
    return val !== 0
      ? { value: val > 0 ? `+${val}` : `${val}`, isPositive: val > 0 }
      : undefined;
  };
